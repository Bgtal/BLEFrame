package com.blq.ssnb.blutoothbleframe.bodyfat;
...
public class BleBodyFatHelper extends AbsBleDeviceHelper {
    ...
    public BleBodyFatHelper(Activity activity) {
        super(activity);
    }

    @Override
    protected IBleDeviceRuler initDeviceRuler() {
        return new BleBodyFatRule();
    }
    ...
    private void dataAvailableResul(final byte[] command) {
        ...
        if (action == BodyFatCmdUtil.CommandAction.SET_DEVICE_TIME.getAction()) {
            setDeviceTimeResult(true);
        } else if (action == BodyFatCmdUtil.CommandAction.SET_DEVICE_TIME.getFailAction()) {
            setDeviceTimeResult(false);
        } else if (action == BodyFatCmdUtil.CommandAction.GET_DEVICE_TIME.getAction()) {
            final Calendar c = Calendar.getInstance();
            int year = BodyFatCmdUtil.ConvertBCD2Decimal(command[1]) + 2000;
            int month = BodyFatCmdUtil.ConvertBCD2Decimal(command[2]) - 1;
            int day = BodyFatCmdUtil.ConvertBCD2Decimal(command[3]);
            int hour = BodyFatCmdUtil.ConvertBCD2Decimal(command[4]);
            int minute = BodyFatCmdUtil.ConvertBCD2Decimal(command[5]);
            int second = BodyFatCmdUtil.ConvertBCD2Decimal(command[6]);
            c.set(year, month, day, hour, minute, second);
            getDeviceTimeResult(true, c);
        } else if (action == BodyFatCmdUtil.CommandAction.GET_DEVICE_TIME.getFailAction()) {
            getDeviceTimeResult(false, Calendar.getInstance());
        } else if (action == BodyFatCmdUtil.CommandAction.SET_BRUSHING_TIME.getAction()) {
            setBrushingTimeResult(true);
        } else if (action == BodyFatCmdUtil.CommandAction.SET_BRUSHING_TIME.getFailAction()) {
            setBrushingTimeResult(false);
        } else if (action == BodyFatCmdUtil.CommandAction.GET_BRUSHING_TIME.getAction()) {
        ...
        } else if (action == BodyFatCmdUtil.CommandAction.GET_BRUSHING_TIME.getFailAction()) {
            getBrushingTimeResult(false, 0, 0, 0, 0);
        } else if (action == BodyFatCmdUtil.CommandAction.GET_HISTORY_BRUSHING_DATA.getAction()) {
            ...
            year = BodyFatCmdUtil.ConvertBCD2Decimal(command[2]) + 2000;
            month = BodyFatCmdUtil.ConvertBCD2Decimal(command[3]) - 1;
            day = BodyFatCmdUtil.ConvertBCD2Decimal(command[4]);

            if (command[5] >= 0x24) {
                //减去0x48
                hours = BodyFatCmdUtil.ConvertBCD2Decimal((byte) (command[5] - 48));
            } else {
                hours = BodyFatCmdUtil.ConvertBCD2Decimal(command[5]);
            }

            minutes = BodyFatCmdUtil.ConvertBCD2Decimal(command[6]);
            seconds = BodyFatCmdUtil.ConvertBCD2Decimal(command[7]);
            ...
            String info = "获取刷牙历史数据:" + "id:" + id + ";时间:" + SnbTimeUtil.date2String("yyyy-MM-dd mm:HH:ss", c.getTime()) + ";time:" + totalTime;

            SnbLog.e(logTag, info);
            setText(info);
...
//            }
        ...
        } else if (action == BodyFatCmdUtil.CommandAction.GET_HISTORY_BRUSHING_DATA.getFailAction()) {

        } else if (action == BodyFatCmdUtil.CommandAction.GET_VERSION.getAction()) {
            final String version = (char) command[1] + "." + (char) command[2] + "."
                    + (char) command[3] + "." + (char) command[3];
            getVersionResult(true, version);
        } else if (action == BodyFatCmdUtil.CommandAction.GET_VERSION.getFailAction()) {
            getVersionResult(false, "0.0.0.0");
        } else if (action == BodyFatCmdUtil.CommandAction.MCU_RESET.getAction()) {
            mcuResetResult(true);
        } else if (action == BodyFatCmdUtil.CommandAction.MCU_RESET.getFailAction()) {
            mcuResetResult(false);
        } else if (action == BodyFatCmdUtil.CommandAction.OTA.getAction()) {
            otaResult(true);
        } else if (action == BodyFatCmdUtil.CommandAction.OTA.getFailAction()) {
            otaResult(false);
        } else if (action == BodyFatCmdUtil.CommandAction.FACTORY_RESET.getAction()) {
            factoryResetResult(true);
        } else if (action == BodyFatCmdUtil.CommandAction.FACTORY_RESET.getFailAction()) {
            factoryResetResult(true);
        } else if (action == BodyFatCmdUtil.CommandAction.GET_BATTERY_PERCENT.getAction()) {

            final int batteryPercent = command[1] & 0xFF;
            getBatteryPercentResult(true, batteryPercent);
        } else if (action == BodyFatCmdUtil.CommandAction.GET_BATTERY_PERCENT.getFailAction()) {
            getBatteryPercentResult(false, 0);
        } else if (action == BodyFatCmdUtil.CommandAction.BRUSHING_GUIDING_PROGRESS.getAction()) {
            brushingGuidingResult(true);
        } else if (action == BodyFatCmdUtil.CommandAction.BRUSHING_GUIDING_PROGRESS.getFailAction()) {
            brushingGuidingResult(false);
        } else if (action == BodyFatCmdUtil.CommandAction.GET_TOOTH_BRUSH_STATUS.getAction()) {
            getToothBrushStatusResult(true, command[1] & 0xFF);
        } else if (action == BodyFatCmdUtil.CommandAction.GET_TOOTH_BRUSH_STATUS.getFailAction()) {
            getToothBrushStatusResult(false, -1);
        } else if (action == BodyFatCmdUtil.CommandAction.SAVE_HISTORY_DATA.getAction()) {
            addHistoryResult(true);
        } else if (action == BodyFatCmdUtil.CommandAction.SAVE_HISTORY_DATA.getFailAction()) {
